...
namespace DND_5e_Character_Generator
{
	public partial class Form1 : Form
	{
		public Form1()
		{
			...
			// Setting element values to a default value in case the user doesn't change anything
			cbxRace.SelectedIndex = 0;
			cbxClass.SelectedIndex = 0;
			cbxRandomize.SelectedIndex = 0;
			...
			// Setting the values for the default race (Dragonborn).
			// This is because the program only changes the values when the combobox index is changed.
			Character.abilityBonuses[(int)Stats.Str] = 2;
			Character.abilityBonuses[(int)Stats.Cha] = 1;
		}
		...
		private void cbxRace_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Utility.CalculateAbilityBonus(cbxRace.Text);
		}
